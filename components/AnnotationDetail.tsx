...
import { en, TYPE_LABELS, formatLocaleDateTime } from '@/lib/i18n';
    ...
    <section data-testid="annotation-detail" className="space-y-4">
      ...
      <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-card)] p-4 shadow-sm">
        ...
        <div className="mt-4 border-t border-[var(--color-border)] pt-4">
          <p className="mb-2 text-xs font-medium text-[var(--color-muted)]">{en.form.description}</p>
          <MarkdownView content={annotation.description ?? ''} emptyText={en.form.noDescription} />
        </div>
        {annotation.imageId && (
          <div className="mt-4 border-t border-[var(--color-border)] pt-4">
            <p className="mb-2 text-xs font-medium text-[var(--color-muted)]">
              {en.actions.screenshotPreview}
            </p>
            <AnnotationImage
